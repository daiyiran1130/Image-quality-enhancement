#!/usr/bin/env python3
"""
Draw fig1.2.1.many.simple.py — Simplified bar chart for MULTI degradation types.
...
Selection criterion: image-count-weighted mean PSNR across the 16 multi scopes.

Layout : 4 rows (metrics) x 16 columns (multi degradation types), 64 subplots.
...
Usage
-----
  python "Draw fig1.2.1.many.simple.py" [input.parquet] [output_stem]
...
# ── Multi degradation types only (contain "+"), sorted by leading type ─────
DEGRADATION_SCOPES = [
    "blur+halo",  "blur+hole",  "blur+spot",
    "color+blur", "color+halo", "color+hole", "color+spot",
    "halo+blur",  "halo+hole",  "halo+spot",
...
def main():
    ...
    stem    = sys.argv[2] if len(sys.argv) > 2 else "Draw fig1.2.1.many.simple"
