#!/usr/bin/env python3
"""
Draw fig1.2.1complex.py — Generate LaTeX/pgfplots bar-chart figure for
degradation-classified image quality metrics.

Layout : 4 rows (metrics) × 21 columns (degradation types), 84 subplots.
Each subplot shows all 20 models as bars (coloured by architecture),
with 95 % CI error bars and significance markers.
Optional connection lines link the same model across degradation-type columns.

Usage
-----
  python "Draw fig1.2.1complex.py" [input.parquet] [output_stem]
...
# Degradation scopes: sorted so singles come first within each leading-type group
DEGRADATION_SCOPES = [
    "blur",       "blur+halo",  "blur+hole",  "blur+spot",
    "color",      "color+blur", "color+halo", "color+hole", "color+spot",
    "halo",       "halo+blur",  "halo+hole",  "halo+spot",
    "hole",       "hole+blur",  "hole+halo",  "hole+spot",
    "spot",       "spot+blur",  "spot+halo",  "spot+hole",
]

# image_count per degradation type (from the parquet, using LQ as reference)
SCOPE_COUNTS = {
    "blur":       700,
    "blur+halo":  406,
    "blur+hole":  363,
...
def _scope_label(scope: str) -> str:
    ...
    # Format degradation name: replace + with \,+\, for nicer spacing
...
def generate_tex(data, show_connections: bool) -> str:
    ...
    for row, (metric, mlabel) in enumerate(zip(METRICS, METRIC_LABELS)):
        ...
        for col, (scope, slabel) in enumerate(zip(DEGRADATION_SCOPES, SCOPE_LABELS)):
            ...
            if row == 0:          # column titles only on top row
                lines += [
                    f"  title={{{slabel}}},",
...
def main():
    ...
    stem    = sys.argv[2] if len(sys.argv) > 2 else "Draw fig1.2.1complex"
