...
# HARALICK spans 0.5–67, log scale keeps outliers (PowerPaint, RSCP2GAN) visible
...
def yrange(data, scope, metric, use_log):
    ...
    if not vals_hi:
        return 0.0, 1.0
    if use_log:
        # bar fills from ymin upward in log space; give generous headroom
        return 0.3, max(vals_hi) * 3.5
    span = max(vals_hi) - min(vals_lo)
    ymin = max(0.0, min(vals_lo) - span * 0.04)
    ymax = max(vals_hi) + span * 0.20   # room for significance markers
    return ymin, ymax
...
def generate_tex(data, show_connections: bool) -> str:
    ...
    # Force standalone to see the full content area
    lines.append(
    ...
    # Group models by architecture (for per-group \addplot with uniform colour)
    ...
    for row, (metric, mlabel) in enumerate(zip(METRICS, METRIC_LABELS)):
        ...
        for col, (scope, slabel) in enumerate(zip(PSNR_SCOPES, SCOPE_LABELS)):
            ...
            # ── bars (one \addplot per architecture group for uniform colour) ─
            ...
            # ── significance markers + named coordinates (connection anchors) ─
            for mi, model in enumerate(MODEL_ORDER):
                d = data.get((scope, model, metric))
                if not d:
                    continue
                ...
                # Coordinate at bar TOP (= data value) for connection lines
                if show_connections:
                    lines.append(
    ...
    # ── Connection lines ──────────────────────────────────────────────────────
    if show_connections:
        lines.append("% ── Connection lines: same model, consecutive PSNR scopes ──────────")
    ...
    # ── Legend ────────────────────────────────────────────────────────────────
    # Bottom of last row: (n_rows-1) * (SUBPLOT_H + GAP_Y) + SUBPLOT_H
...
def main():
    ...
    stem    = sys.argv[2] if len(sys.argv) > 2 else "figure2"
